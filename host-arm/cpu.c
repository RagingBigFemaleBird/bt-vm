...
#define __PB_ENTRIES (BT_CACHE_CAPACITY * 2)
...
h_bt_cache_restore(struct v_world *world)
{
    ...
    if (total != 0 && set != 0) {
        ...
        if (!(world->poi->type & V_INST_CB)) {
            ...
            h_set_bp(world, world->poi->addr, 0);
        } else {
    ...
    if (count != 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (!(plan[i].poi->type & V_INST_CB)) {
                hcache[i].bvr[j] = plan[i].poi->addr;
...
h_switcher(unsigned long trbase, struct v_world *w)
{
    ...
    asm volatile ("mcr p15, 0, %0, c1, c0, 0"::"r" (hostcpu.p15_ctrl &
            (~P15_CTRL_V)));
    asm volatile ("mcr p15, 0, %0, c12, c0, 0"::"r" ((((unsigned int)
                    h_vector_entry) & H_POFF_MASK) | (((unsigned int) (h->hcpu.
                        switcher)) & H_PFN_MASK)));
...
/*        "h_vector_test:\n\r" "cpsie i\n\r" "1:\n\r" "b 1b\n\r"*/
        ...
        "h_vector_return:\n\r" CACHE_BT_QUICKPATH(function_no)
        "mrs r0, spsr\n\t"
/*debug        "mov r0, #0xff\n\t"*/
...
/*              return 1;*/
        ...
        if (!((w->gregs.cpsr & G_PRIV_MASK) == G_PRIV_RST)) {
            if (umtest == 0 && h->gcpu.r6 == 0xdeadbeef) {
                ...
                umtest = 1;
            } else if (umtest > 0 && h->gcpu.r6 == 0xdeadbeef) {
                umtest++;
                if (h->gcpu.pc == 0x8110 || umtest >= 0x9998) {
                    ...
                    V_ERR("%x \n  - %x\n ^^^", umtest1, umtest2);
                    umtest = 0;
                    for (ii = 0; ii < V_PERF_COUNT; ii++)
                        V_ERR("VM perf %x: %lx", ii, v_perf_get(ii));
                    for (ii = 0; ii < H_PERF_COUNT; ii++)
                        V_ERR("Host perf %x: %lx", ii, h_perf_get(ii));
                    for (ii = 0; ii < H_TSC_COUNT; ii++)
                        V_ERR("TSC %x: %llx", ii, h_tsc_get(ii));
                }
            } else {
                V_ERR("Guest sys call @ %x, with %x %x %x", h->gcpu.pc,
                    h->gcpu.r0, h->gcpu.r1, h->gcpu.r2);
            }
            h_inject_int(w, 2);
            h_perf_tsc_end(H_PERF_TSC_PI, 0);
        }
